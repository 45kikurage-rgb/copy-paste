...
URL自動登録では、解析で取得した商品名を source_name として省略せずデータベースへ保持します。自己利用分のカードに表示する name は別項目です。

初回登録時は商品画像と、商品名・容量・引換先・引換期限を確認画面に表示します。抽出商品名が15文字以上なら「見やすいように商品名を編集しましょう」と表示し、利用者がカード表示名だけを短くできます。変更しても source_name は変更しません。

2件目以降は、抽出商品名＋容量＋引換先＋引換期限が一致する既存カードを検索します。一致した場合は商品画像を再提示せず「〇〇カードに登録します」と表示し、OK/NG確認後に追加します。カード表示名を変更済みの場合は、その変更後の表示名を使います。引換期限が異なる場合は別カードです。
