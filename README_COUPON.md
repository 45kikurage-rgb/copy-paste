...
「自己利用分」→「編集」→「このクーポンを削除」で、表示カードだけでなくD1のクーポン本体・期限・個別URL/画像・予約履歴・再解析状態を削除します。R2の代表画像と利用用画像も削除対象です。予約中のカードは誤削除防止のため削除できません。

## スターバックスeGift

`https://gift.starbucks.co.jp/e/...` のURLを共有した場合、金額・有効期限を公式ページから直接読み取り、`スタバ500` / `スタバ700` のようなカードへ自動登録します。同額の既存カードがあれば新規カードを作らず、そのカードへ個別URLを追加します。既存のStarbucks URL型カードも再整理対象です。

## コメダeGift

`https://komeda.e-gift.co/c/.../...` のURLを共有した場合、公式eGiftページから金額・有効期限・券面画像を解析し、`1000円 コメダコーヒー` のような既存カードへ統合します。同じURLは重複登録しません。既存のコメダURL型カードも再整理対象です。

## ローソンeGift

`https://lawson-i.e-gift.co/c/.../...` のURLを共有した場合、公式eGiftページから商品名または金額・有効期限・券面画像を解析して自動登録します。金額型の券は `500円 ローソン` のようにまとめ、商品型の券は商品名で管理します。`gift_wallet_url` は解析には使わず、公式eGift本体だけを取得します。同じURLは重複登録せず、既存のローソンURL型カードも再整理対象です。

## ミスタードーナツeGift

`https://misterdonut.e-gift.co/c/.../...` は専用解析を行います。`gift_wallet_url` は無視して公式eGift本体だけを取得し、商品名/金額・有効期限・券面画像を解析します。金額型のギフトチケットは `200円 ミスタードーナツ` のように正規化し、同額の既存カードへ統合します。既存のミスタードーナツURL型カードも再整理対象です。


## URLクーポンの同一判定

自動登録するURL型クーポンは「商品名＋容量＋引換先＋引換期限」の4項目を同一判定キーにします。4項目が一致した場合だけ同じカードへ追加し、期限が異なる場合は必ず別カードにします。容量表記がない商品は容量を空欄として扱い、商品名・引換先・期限で区別します。既存URLは1件ずつ再解析して、誤って大きなカテゴリへまとめられたカードから正しいカードへ移動します。
